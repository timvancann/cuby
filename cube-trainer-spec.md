...
- The case library for v1 is the 57 OLL cases, grouped as in the source PDF ("How to Learn OLL in One Month", CubeHead): Oriented Edges (OCLL), T-shapes, Squares, Solved Corners, Lightning Bolts, P-shapes, C-shapes, Fishes, L-shapes, W-shapes, Lines, Knight Moves, Awkward Shapes, Dot cases — 14 ownership groups. The PDF's "Easy Algs" page is a preview of 8 cases that all also appear in their shape groups; since a case belongs to exactly one group (§2), those cases carry an `easy` flag instead, usable as a selection preset. The groups double as the learning order, so selection maps naturally onto "the groups I've learned so far".
